...
# 5. 如果要注入动态库，将生成的动态库（脚本中默认命名SignFramework.framework）放入到new文件夹；
#
...
if [ -n "$Framework_Path" ]; then 
	...
	# 需要注入的动态库的路径（这里写死名称了：SignFramework.framework/SignFramework）
	INJECT_FRAMEWORK_RELATIVE_PATH="Frameworks/SignFramework.framework/SignFramework"
	## 通过工具实现注入
	# yololib MachO名称
...
# 6. 对APP签名
codesign -f -s "$EXPANDED_CODE_SIGN_IDENTITY" --entitlements=${Work_Path}/entitlements.plist "Payload/${App_Binary}.app"
